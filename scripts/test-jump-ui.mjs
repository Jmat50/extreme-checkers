/**
 * Browser regression for jump targeting / multi-jump chains.
 *
 * Local 2-player, default rules. Scripted line:
 *   1. red  (2,3) -> (3,4)  via DRAG
 *   2. black (5,4) -> (4,5) via click
 *   3. red may jump: (3,4) x (4,5) -> lands (5,6) (captures optional)
 *      -> chain continues: (5,6) x (6,5) -> lands (7,4) which is a hazard
 *      -> red piece explodes, turn passes to black.
 *
 * Verifies: adjacent jump landing is highlighted and clickable; mid-chain
 * continuation stays forced once a jump is started; hazards eliminate.
 *
...
async function main() {
  ...
  // 3. Capture is optional: jumper offers (5,6) among targets (may also have slides)
  const draggableCount = await page.locator('.piece-sprite--draggable').count();
  assert(draggableCount >= 1, `optional capture: at least 1 draggable red piece (got ${draggableCount})`);
  ...
  assert(
    targets.some((s) => s.row === 5 && s.col === 6),
    `jump: adjacent landing (5,6) is among highlighted targets (got ${JSON.stringify(targets)})`,
  );
